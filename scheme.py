...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        operands = rest.map(lambda x: scheme_eval(x, env)) # Pair.map
...
class Frame(object):
    ...
    def lookup(self, symbol):
        ...
        if symbol in self.bindings:
            ...
            return self.parent.lookup(symbol) # there maybe more than one parent frames
        # END PROBLEM 3
    ...
    def make_child_frame(self, formals, vals):
        ...
        # BEGIN PROBLEM 11
        "*** YOUR CODE HERE ***"
        # Pair has the method __len__(self)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(symbol, LambdaProcedure(target.second, body, env))
        return symbol
        # END PROBLEM 10
    else:
...
def do_and_form(expressions, env):
    ...
    if expressions is nil:
        return True
    if expressions.second is nil:
        return scheme_eval(expressions.first, env, True) # last sub-expression
...
def do_define_macro(expressions, env):
    ...
    "*** YOUR CODE HERE ***"
    check_form(expressions, 2)
    target = expressions.first # (for formal iterable body)
...
# sub-expressions 2 and 3 in if
# the last expression in or, and, cond, begin
